                ...
                <b-col md="10" class="py-3 mx-auto py-md-0">
                    <b-card header-tag="header" class="capturephoto-form">
                        ...
                        <b-overlay :show="cameraLoading" rounded="sm">
                            ...
                            <template v-else>
                                <div class="video-camera-player canvas-wrapper">
                                    <canvas ref="canvasCaptured" id="canvasImagePhoto" class="canvas-preview" :class="photoCaptured ? 'active' : 'hidden'"></canvas>
                                    <canvas ref="canvas" id="canvasWebCamera" class="canvas-camera"></canvas>
                                    <video ref="camera" id="videoWebCamera" :show="openVideoCamera()" allow="camera;microphone" class="silhouetteVideo video-camera" playsinline="" autoplay muted></video>
...
@section('vuestyle')
    <style>
        ...
        .video-camera-player .canvas-camera {
            clear: both;
            display: block;
            position: relative;
            margin: 0 auto;
        ...
        .video-camera-player .video-camera {
            display: block;
            position: absolute;
            top: 0;
            margin: 0 auto;
...
@section('vuemethod'){
    init: function() {
        ...
        this.canvasCameraPlayer = this.$refs.canvas;
        this.canvasImagePhoto = this.$refs.canvasCaptured; 
    ...
    openVideoCamera: function() {
        var $this = this;
        mediaSDaiLover.enableCamera(this.videoCameraPlayer, this.canvasCameraPlayer, function(result) {
            if (result.success) {
